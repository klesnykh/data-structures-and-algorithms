...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => arr.filter(val => typeof(val)==='number'))
  .map(arr => arr.filter(num => num%5===0))
  .map(arr => arr.map(num => 2**num));
};
...
let findShortest = (data) => {
  ...
  return data.filter(obj => obj.height===curHeight)[0].name;
};
