...
const screenForNames = (arr) => {
  let regex = /^((Mr\.\s)|(Mrs\.\s)|(Ms\.\s)|(Dr\.\s))[aA-zZ]{1,}$/gm;
...
let biggerThanLuke = (arr) => {
  if(arr.length>0){
    let lukeMass = parseInt(arr.find(obj => obj.name==='Luke Skywalker').mass);
    return arr.filter(obj => parseInt(obj.mass)>lukeMass).map(obj=>obj.name).join(' - ');
  } else{return '';}
};
