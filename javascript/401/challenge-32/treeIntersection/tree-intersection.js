...
function tree_intersection (tree1, tree2){
  let map = {};
  let vals = [...breadthFirst(tree1), ...breadthFirst(tree2)];
  //console.log(vals);
  let sameVals = [];

  for(let i=0; i<vals.length; i++){
    if(map[vals[i]]===1){
      sameVals.push(vals[i]);
      map[vals[i]]++;
    }
    else{
      if(map[vals[i]]){
...
module.exports = {BinaryTree, BinarySearchTree, Node, breadthFirst, treeFizzBuzz, tree_intersection};
